...
inline float4	color_texture(__global t_primitive *prim, t_texture tex, float4 normal, t_img_info info, __global int *raw_bmp)
{
		...
		__global t_img_info *img_info, __global int *raw_bmp)
{
	...
	for (cur_l = 0; cur_l < argn->nb_lights; cur_l++)
	{
		...
		{
			t_img_info info = img_info[mat->texture.info_index];
